...
function OpenWipeMenu()
    ...
    RMenu.Add("wipe", "wipe_playerlist", RageUI.CreateMenu("Wipe","Wipe menu"))
    RMenu:Get('wipe', 'wipe_playerlist').Closed = function()
        inMenu = false
    end

    RMenu.Add('wipe', 'wipe_confirm', RageUI.CreateSubMenu(RMenu:Get('wipe', 'wipe_playerlist'), "Confirmation", "Voulez-vous vraiment effacer ce Joueur ?"))
            ...
            while inMenu do
                ...
                RageUI.IsVisible(RMenu:Get("wipe",'wipe_playerlist'), true, true, true, function()
                    for k,v in pairs(GetActivePlayers()) do 
                        RageUI.ButtonWithStyle(GetPlayerServerId(v).." - "..GetPlayerName(v), nil, { RightLabel = ">> Wipe le Joueur" }, true, function(Hovered, Active, Selected)
                            if Selected then
